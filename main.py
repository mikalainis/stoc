...
class DiscoveryAgent:
    ...
    def _check_ticker_criteria(self, ticker: str) -> bool:
        ...
        """
        try:
            # Initialize vars for safe logging in case of early exit
            rev_growth = 0.0
            pe = 0.0
            gross_margin = 0.0
            debt_equity = 100.0
            ...
            if not isinstance(g_data, list) or not g_data:
                pass # Continue to allow partial logging if possible, though logic usually needs all
            else:
                rev_growth = g_data[0].get('revenueGrowth', 0) or 0

            # 3. Check FCF to Sales
            ...
            # EVALUATE
            pass_pe = pe > 15
            pass_gm = gross_margin > 0.20
            pass_de = debt_equity <= 1.0
            pass_roe = roe > 0.12
            pass_growth = rev_growth > 0.04
            pass_fcf = fcf_sales > 0.05

            if pass_pe and pass_gm and pass_de and pass_roe and pass_growth and pass_fcf:
                ...
                # Explicit rejection logging for visibility in test mode
                if "--test" in sys.argv:
                     print(f"   ❌ SKIP: {ticker}\n"
                           f"      Growth: {rev_growth:.1%} ({'✅' if pass_growth else '❌ < 4%'})\n"
                           f"      PE Ratio: {pe:.1f} ({'✅' if pass_pe else '❌ > 15'})\n"
                           f"      Gross Margin: {gross_margin:.1%} ({'✅' if pass_gm else '❌ > 20%'})\n"
                           f"      Debt/Equity: {debt_equity:.2f} ({'✅' if pass_de else '❌ <= 1.0'})\n"
                           f"      ROE: {roe:.1%} ({'✅' if pass_roe else '❌ > 12%'})\n"
                           f"      FCF/Sales: {fcf_sales:.1%} ({'✅' if pass_fcf else '❌ > 5%'})")
                return False
...
class PortfolioAudit:
    ...
    def scan(self):
        ...
        try:
            ...
            # In test mode with dummy secrets, Alpaca might fail. We should allow discovery scan to proceed if it's test mode.
            if "--test" in sys.argv:
                print(f"   ⚠️ Alpaca Error (Expected in Test Mode): {e}")
                portfolio_map = {}
            else:
        ...
        if not portfolio_map:
            print(f"   ⚠️ Portfolio Empty (or Test Mode). Switching to Discovery Mode...")
            print(f"   🔍 DEBUG: Calling discovery.find_top_picks()...")
            candidates = self.discovery.find_top_picks()
            print(f"   🔍 DEBUG: find_top_picks returned {len(candidates)} candidates.")
